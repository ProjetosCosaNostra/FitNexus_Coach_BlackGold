...
$FailureClassObservation = 'BGF-STAGE35-ALERT-SECRET-BOOTSTRAP-OBSERVATION-298'
...
function Fail-Closed {
  ...
  Write-Error "STAGE35_ALERT_SECRET_BOOTSTRAP_V2=FAIL FAILURE_CLASS=$FailureClass CODE=$Code"
  exit 1
}
...
function Invoke-SupabaseCli {
  ...
  if ($script:SupabaseCommandKind -eq 'direct') {
    $null = & $script:SupabaseCommand @Arguments 2>&1
  } else {
    $null = & $script:SupabaseCommand --yes 'supabase@latest' @Arguments 2>&1
  }
  return $LASTEXITCODE
}
...
if ($null -ne $supabase) {
  $script:SupabaseCommandKind = 'direct'; $script:SupabaseCommand = $supabase.Source
} else {
...
$oldToken = [Environment]::GetEnvironmentVariable('SUPABASE_ACCESS_TOKEN','Process')
$tempEnvFile = $null
$runtimeWritten = $false
$githubWritten = $false
try {
  $tempEnvFile = Join-Path ([IO.Path]::GetTempPath()) ("fitnexus-stage35-alert-secrets-v2-{0}.env" -f ([Guid]::NewGuid().ToString('N')))
  $envText = @(
  ...
  } catch { Fail-Closed $FailureClassObservation 'SUPABASE_RUNTIME_SECRET_NAME_OBSERVATION_FAILED' }
  $observedNames = @($secretProbe | ForEach-Object { if ($_.PSObject.Properties.Name -contains 'name') { [string]$_.name } })
  foreach ($name in $RuntimeNames) {
    if ($observedNames -notcontains $name) { Fail-Closed $FailureClassObservation "SUPABASE_RUNTIME_SECRET_MISSING_$name" }
  }
  ...
  [Environment]::SetEnvironmentVariable('SUPABASE_ACCESS_TOKEN',$oldToken,'Process')
...
if ($null -ne $tempEnvFile -and (Test-Path -LiteralPath $tempEnvFile)) { Fail-Closed $FailureClassResidue 'TEMP_SECRET_ENV_FILE_REMAINS' }
if (-not $runtimeWritten -or -not $githubWritten) { Fail-Closed $FailureClassPartial 'SECRET_BOOTSTRAP_NOT_COMPLETE' }

Write-Host 'STAGE35_ALERT_SECRET_BOOTSTRAP_V2=PASS'
Write-Host 'GITHUB_ACTIONS_SECRET_NAMES_VERIFIED=4/4'
Write-Host 'SUPABASE_EDGE_RUNTIME_SECRET_NAMES_VERIFIED=3/3'
Write-Host 'RUNTIME_SECRET_VERIFICATION=SUPABASE_MANAGEMENT_API_NAMES_ONLY'
