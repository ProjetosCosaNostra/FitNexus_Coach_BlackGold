...
from stage40_billing_migration_frontier import json_dump, state, to_promotion

ROOT = Path(__file__).resolve().parents[2]
LEDGER = ROOT / "04_backend_supabase" / "migration_ledger_authority.json"
PROMOTION_GUARD = ROOT / "04_backend_supabase" / "tools" / "verify_stage40_billing_production_environment_interlock_promotion.py"


def fail(detail: str) -> None:
    raise SystemExit(
        "STAGE40_PROMOTION_HISTORICAL_COMPAT=FAIL\n"
        "FAILURE_CLASS=BGF-STAGE40-HISTORICAL-GUARD-FUTURE-LEDGER-PINNING-366\n"
        f"DETAIL={detail}"
    )
...
def main() -> None:
    ...
    try:
        ...
        if result.returncode != 0:
            fail(f"sealed Stage40 promotion guard failed under historical projection: exit={result.returncode}")
    except ValueError as exc:
        fail(str(exc))
    finally:
        LEDGER.write_bytes(original)
    print("STAGE40_PROMOTION_HISTORICAL_COMPAT=PASS")
