...
FAILURE_CLASS = "BGF-STAGE42-PRIVACY-NOTICE-RECEIPT-STRUCTURAL-REVIEW-384"
SHA256_RE = re.compile(r"^[0-9a-f]{64}$")


def fail(detail: str) -> None:
    raise SystemExit(
        "STAGE42_PRIVACY_NOTICE_RECEIPT_REVIEW=FAIL\n"
...
def main() -> None:
    ...
    for key in (
        "legal_privacy_review_artifact_digest",
        "approved_document_sha256_digest",
        "processor_subprocessor_inventory_digest",
        "retention_matrix_digest",
        "international_transfer_review_digest",
        "encarregado_or_exemption_contact_review_digest",
    ):
    ...
    version = str(receipt.get("approved_privacy_notice_version", "")).strip()
    ...
    if not version or len(version) > 120:
        fail("approved privacy notice version missing or malformed")
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", effective) is None:
        fail("effective date must be YYYY-MM-DD")

    url = str(receipt.get("stable_published_privacy_url", "")).strip()
    parsed = urlparse(url)
    host = (parsed.hostname or "").lower()
    if parsed.scheme != "https" or not host:
        fail("published privacy URL must be absolute HTTPS")
    if host in {"localhost", "127.0.0.1", "::1", "example.com", "www.example.com"}:
        fail("published privacy URL is placeholder/local")
    if re.search(r"(^|\.)(test|staging|preview|dev)(\.|$)", host):
        fail("published privacy URL appears non-production")
    if re.search(r"example|placeholder|localhost|preview", url, re.IGNORECASE):
        fail("published privacy URL contains placeholder marker")
    ...
    print("STAGE42_PRIVACY_NOTICE_RECEIPT_REVIEW=PASS_STRUCTURAL_CANDIDATE_ONLY")
    print("LEGAL_PRIVACY_REVIEW_VERIFIED_BY_SCRIPT=false")
    print("PUBLISHED_CONTENT_LEGAL_SUFFICIENCY_VERIFIED_BY_SCRIPT=false")
    print("TRANSFER_APPLICABILITY_LEGAL_CONCLUSION_VERIFIED_BY_SCRIPT=false")
