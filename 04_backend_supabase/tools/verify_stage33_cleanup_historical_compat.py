...
CURRENT_BASELINE = "e30aa197fe5d19b9e385a8720944c6c9c10d34ee"
CURRENT_OBSERVED = "2026-08-22T06:11:53.105067Z"
...
def project_ledger(current: dict) -> dict:
    ...
    if remote.get(CLEANUP_NAME) != CLEANUP_VERSION:
        fail("current Stage33 remote cleanup receipt missing")
    repo_only = [
        row for row in current.get("declared_divergences", [])
        if isinstance(row, dict) and row.get("direction") == "repo_only"
    ]
    if repo_only:
        fail("remote-complete cleanup lifecycle retained repo-only divergence")
    ...
    value["remote_migrations"] = [
        ...
        if not (
            isinstance(row, dict)
            and row.get("name") in {REVOCATION_NAME, CLEANUP_NAME}
        )
    ]
    value["declared_divergences"].append({
        "direction": "repo_only",
        "name": REVOCATION_NAME,
        "reason": "Historical Stage33 pre-remote projection used only for immutable guard replay after the real revocation and cleanup were both applied and verified.",
...
def run(mode: str) -> None:
    ...
    # Always prove the actual remote-complete Stage33 frontier before projecting history.
    ...
    with tempfile.TemporaryDirectory(prefix="fitnexus-stage33-cleanup-history-") as tmp:
        ...
        try:
            ...
            if mode == "assessment":
                # The immutable assessment dynamically enumerates Stage33 revocation files.
                # Hide the later cleanup migration from that historical filesystem view.
    ...
    print(f"MODE={mode}")
    print("ACTUAL_STAGE33_STATE=POST_REVOCATION_EDGE_PROOF_VERIFIED_CLEANUP_REMOTE_COMPLETE")
