...
HIDDEN = BACKEND / ".stage53_billing_evidence_provider_fk_index_hardening.promotion-hidden"
CANDIDATE_GUARD = BACKEND / "tools/verify_stage53_billing_evidence_provider_fk_index_candidate.py"
FAILURE_CLASS = "BGF-STAGE53-HISTORICAL-GUARD-FUTURE-MIGRATION-PINNING-501"


def fail(detail: str) -> None:
    raise SystemExit(
        "STAGE53_CANDIDATE_HISTORICAL_COMPAT=FAIL\n"
...
def main() -> None:
    ...
    try:
        ...
        result = subprocess.run([sys.executable, str(CANDIDATE_GUARD)], cwd=ROOT, check=False)
        if result.returncode != 0:
            fail(f"sealed Stage53 candidate guard failed under historical projection: exit={result.returncode}")
    finally:
        if HIDDEN.exists():
    ...
    print("STAGE53_CANDIDATE_HISTORICAL_COMPAT=PASS")
