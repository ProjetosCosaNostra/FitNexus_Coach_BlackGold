-- CANDIDATE ONLY — promote through a controlled migration after CI.
-- BGF-BILLING-RECURRING-LIFECYCLE-AUTHORITY-V1
--
-- Purpose:
-- 1) bind the Asaas subscription created after a paid recurring Checkout;
-- 2) retain the monthly/yearly billing interval as server authority;
-- 3) reconcile recurring PAYMENT_* and provider-side subscription termination;
-- 4) fail closed when customer/subscription correlation is ambiguous.
...
begin
  ...
  -- The Checkout webhook is expected to bind provider_customer_ref first.
  -- We do not assume that Checkout.externalReference is copied to the Asaas
  -- subscription. Correlation therefore requires exactly one paid FitNexus
  -- organization for the provider customer AND a completed checkout with the
  -- same plan and billing interval. Ambiguity fails closed.
...
begin
  ...
  -- Out-of-order historical events must never downgrade a newer paid period.
  ...
  if v_event_type in ('PAYMENT_CONFIRMED','PAYMENT_RECEIVED') then
    ...
    -- Refund/chargeback only affects access if it belongs to the currently
    -- authoritative period. Historical reversals remain auditable but do not
    -- revoke a newer paid period.
