-- Stage 32 recovery R1: rearm only the exact expired synthetic fixture after the
...
--
-- This is a repository-first DML operation, not a schema migration. Execute through
-- Supabase.execute_sql only after CI + merge. It is fail-closed, synthetic-only, and
-- may only extend the expiry of the already-existing exact Stage 32 link.
do $$
declare
