...
function Get-PngDimensions {
    ...
    if ($bytes.Length -lt 24) {
        throw 'FNX_PLAY_SCREENSHOT_02_PNG_TOO_SMALL'
    }

    $signature = @(137, 80, 78, 71, 13, 10, 26, 10)
    for ($i = 0; $i -lt $signature.Count; $i++) {
        if ([int]$bytes[$i] -ne [int]$signature[$i]) {
            throw 'FNX_PLAY_SCREENSHOT_02_INVALID_PNG_SIGNATURE'
        }
    }
...
function Resolve-AdbExecutable {
    ...
    if ($candidates.Count -eq 0) {
        throw 'FNX_PLAY_SCREENSHOT_02_ADB_NOT_FOUND'
    }
...
function Assert-CanonicalReleaseInstalled {
    ...
    $versionName = ''
    $versionCode = ''
    if ($dump.StdOut -match 'versionName=([^\s]+)') {
        $versionName = [string]$Matches[1]
    }
    if ($dump.StdOut -match 'versionCode=(\d+)') {
        $versionCode = [string]$Matches[1]
    }
...
$ContractPath = Join-Path $AndroidDir 'play_store\PLAY_STORE_SCREENSHOT_02_CAPTURE_V1.json'
$StoreEntrypoint = Join-Path $AppRoot 'lib\store_capture_main.dart'
$DataRepositoryPath = Join-Path $AppRoot 'lib\features\professor\professor_data_repository.dart'
$KeyPropertiesFile = Join-Path $AndroidDir 'key.properties'
$Marker = '# GENERATED_BY=FITNEXUS_PLAY_STORE_SCREENSHOT_02_CAPTURE_V1'
...
if (-not (Test-Path -LiteralPath $ContractPath -PathType Leaf)) {
    throw 'FNX_PLAY_SCREENSHOT_02_CONTRACT_MISSING'
}
if (-not (Test-Path -LiteralPath $StoreEntrypoint -PathType Leaf)) {
    throw 'FNX_PLAY_SCREENSHOT_02_ENTRYPOINT_MISSING'
}
if (-not (Test-Path -LiteralPath $DataRepositoryPath -PathType Leaf)) {
    throw 'FNX_PLAY_SCREENSHOT_02_DATA_REPOSITORY_MISSING'
}

$Contract = Get-Content -LiteralPath $ContractPath -Raw | ConvertFrom-Json
if ([string]$Contract.application_id -ne $PackageId) {
    throw 'FNX_PLAY_SCREENSHOT_02_PACKAGE_DRIFT'
}
if ([string]$Contract.capture.dart_define_shot -ne $ShotDefine) {
    throw 'FNX_PLAY_SCREENSHOT_02_SHOT_DEFINE_DRIFT'
}
if ([int]$Contract.capture.target_width_px -ne $TargetWidth -or [int]$Contract.capture.target_height_px -ne $TargetHeight) {
    throw 'FNX_PLAY_SCREENSHOT_02_DIMENSION_CONTRACT_DRIFT'
}
foreach ($property in $Contract.hard_boundaries.PSObject.Properties) {
    if ([bool]$property.Value) {
        throw ('FNX_PLAY_SCREENSHOT_02_BOUNDARY_DRIFT_' + $property.Name)
    }
}

$entrypointText = Get-Content -LiteralPath $StoreEntrypoint -Raw
$dataRepositoryText = Get-Content -LiteralPath $DataRepositoryPath -Raw
if ($entrypointText -notmatch 'FITNEXUS_STORE_CAPTURE_SHOT') {
    throw 'FNX_PLAY_SCREENSHOT_02_SHOT_ROUTING_MISSING'
}
if ($entrypointText -notmatch 'StudentAccessManagementPage') {
    throw 'FNX_PLAY_SCREENSHOT_02_TARGET_WIDGET_MISSING'
}
if ($entrypointText -match 'Supabase\.initialize') {
    throw 'FNX_PLAY_SCREENSHOT_02_CAPTURE_ENTRYPOINT_REMOTE_INIT_FORBIDDEN'
}
if ($dataRepositoryText -notmatch 'fitNexusStoreCaptureDataMode') {
    throw 'FNX_PLAY_SCREENSHOT_02_SYNTHETIC_GATE_MISSING'
}
if ($dataRepositoryText -notmatch 'STORE_CAPTURE_REMOTE_MUTATION_FORBIDDEN') {
    throw 'FNX_PLAY_SCREENSHOT_02_REMOTE_MUTATION_GUARD_MISSING'
}

if ($ValidateOnly) {
    Write-Output 'FITNEXUS_PLAY_STORE_SCREENSHOT_02_CAPTURE_VALIDATE_ONLY=PASS'
    Write-Output ('APPLICATION_ID=' + $PackageId)
    Write-Output 'TARGET=1080x1920'
    Write-Output 'SHOT=02'
    Write-Output 'SYNTHETIC_DATA=true'
    Write-Output 'PRODUCTION_UI_WIDGET=StudentAccessManagementPage'
...
$productionApk = $null
$restoreFailure = $null
$remoteScreenshot = '/sdcard/Download/fitnexus_play_screenshot_02.png'
$scratch = Join-Path $env:TEMP ('FNX_PLAY_SCREENSHOT_02_' + [guid]::NewGuid().ToString('N'))
$failure = $null
$screenshotPath = $null
$receiptPath = $null
$receiptSha = ''
...
try {
    if ($env:OS -ne 'Windows_NT') {
        throw 'FNX_PLAY_SCREENSHOT_02_WINDOWS_REQUIRED'
    }
    if ([string]::IsNullOrWhiteSpace($env:USERPROFILE)) {
        throw 'FNX_PLAY_SCREENSHOT_02_USERPROFILE_MISSING'
    }
    ...
    Assert-NativeSuccess -Result $devicesResult -FailureClass 'FNX_PLAY_SCREENSHOT_02_ADB_DEVICES'
    $serials = @()
    ...
    if ($serials.Count -ne 1) {
        throw ('FNX_PLAY_SCREENSHOT_02_DEVICE_COUNT_' + $serials.Count)
    }
    $serial = [string]$serials[0]

    $sizeBefore = Invoke-NativeCapture -FilePath $adb -Arguments @('-s', $serial, 'shell', 'wm', 'size')
    Assert-NativeSuccess -Result $sizeBefore -FailureClass 'FNX_PLAY_SCREENSHOT_02_WM_SIZE_BEFORE'
    ...
    Assert-NativeSuccess -Result $setSize -FailureClass 'FNX_PLAY_SCREENSHOT_02_WM_SIZE_SET'
    $displayChanged = $true

    $sizeAfter = Invoke-NativeCapture -FilePath $adb -Arguments @('-s', $serial, 'shell', 'wm', 'size')
    Assert-NativeSuccess -Result $sizeAfter -FailureClass 'FNX_PLAY_SCREENSHOT_02_WM_SIZE_AFTER'
    if ($sizeAfter.StdOut -notmatch ('Override size:\s*' + $TargetWidth + 'x' + $TargetHeight)) {
        throw 'FNX_PLAY_SCREENSHOT_02_CAPTURE_SIZE_NOT_APPLIED'
    }

    $flutter = Get-Command flutter -ErrorAction SilentlyContinue
    if ($null -eq $flutter) {
        throw 'FNX_PLAY_SCREENSHOT_02_FLUTTER_NOT_FOUND'
    }
    ...
    if (-not (Test-Path -LiteralPath $KeystoreFile -PathType Leaf) -or -not (Test-Path -LiteralPath $ProtectedSecretFile -PathType Leaf)) {
        throw 'FNX_PLAY_SCREENSHOT_02_SIGNING_AUTHORITY_MISSING'
    }
    ...
    Write-Output 'SCREENSHOT_02_PHASE=FLUTTER_PUB_GET'
    $pubGet = Invoke-NativeCapture -FilePath $flutter.Source -Arguments @('pub', 'get') -WorkingDirectory $AppRoot
    if ($pubGet.ExitCode -ne 0) {
        throw ('FNX_PLAY_SCREENSHOT_02_FLUTTER_PUB_GET_EXIT_' + $pubGet.ExitCode)
    }

    Write-Output 'SCREENSHOT_02_PHASE=BUILD_PRODUCTION_RESTORE_APK'
    ...
    if ($productionBuild.ExitCode -ne 0) {
        throw ('FNX_PLAY_SCREENSHOT_02_PRODUCTION_APK_BUILD_EXIT_' + $productionBuild.ExitCode)
    }
    ...
    if (-not (Test-Path -LiteralPath $builtApk -PathType Leaf)) {
        throw 'FNX_PLAY_SCREENSHOT_02_PRODUCTION_APK_MISSING'
    }
    ...
    Write-Output 'SCREENSHOT_02_PHASE=BUILD_CAPTURE_APK'
    $captureBuild = Invoke-NativeCapture -FilePath $flutter.Source -Arguments @(
        ...
        '--dart-define=FITNEXUS_STORE_CAPTURE_SHOT=02'
    ) -WorkingDirectory $AppRoot
    if ($captureBuild.ExitCode -ne 0) {
        throw ('FNX_PLAY_SCREENSHOT_02_CAPTURE_APK_BUILD_EXIT_' + $captureBuild.ExitCode)
    }
    if (-not (Test-Path -LiteralPath $builtApk -PathType Leaf)) {
        throw 'FNX_PLAY_SCREENSHOT_02_CAPTURE_APK_MISSING'
    }
    $captureApk = Join-Path $scratch 'fitnexus-store-capture-02.apk'
    Copy-Item -LiteralPath $builtApk -Destination $captureApk -Force

    Write-Output 'SCREENSHOT_02_PHASE=INSTALL_CAPTURE_APK'
    Install-Apk -Adb $adb -Serial $serial -Apk $captureApk -FailureClass 'FNX_PLAY_SCREENSHOT_02_CAPTURE_INSTALL_FAILED'
    $captureInstalled = $true

    $forceStop = Invoke-NativeCapture -FilePath $adb -Arguments @('-s', $serial, 'shell', 'am', 'force-stop', $PackageId)
    Assert-NativeSuccess -Result $forceStop -FailureClass 'FNX_PLAY_SCREENSHOT_02_FORCE_STOP'
    ...
    if ([string]::IsNullOrWhiteSpace($documents)) {
        throw 'FNX_PLAY_SCREENSHOT_02_DOCUMENTS_UNRESOLVED'
    }
    $current = Join-Path $documents 'FitNexus_Coach_BlackGold_EXTERNAL\play_store_assets\current'
    $screenshotsDir = Join-Path $current 'screenshots'
    $receiptsDir = Join-Path $current 'receipts'
    New-Item -ItemType Directory -Path $screenshotsDir -Force | Out-Null
    New-Item -ItemType Directory -Path $receiptsDir -Force | Out-Null
    $screenshotPath = Join-Path $screenshotsDir ([string]$Contract.capture.output_filename)
    $receiptPath = Join-Path $receiptsDir 'FITNEXUS_PLAY_STORE_SCREENSHOT_02_RECEIPT_V1.json'
    Remove-Item -LiteralPath $screenshotPath -Force -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $receiptPath -Force -ErrorAction SilentlyContinue

    Write-Output 'SCREENSHOT_02_PHASE=CAPTURE_DEVICE_PNG'
    $capture = Invoke-NativeCapture -FilePath $adb -Arguments @('-s', $serial, 'shell', 'screencap', '-p', $remoteScreenshot)
    Assert-NativeSuccess -Result $capture -FailureClass 'FNX_PLAY_SCREENSHOT_02_SCREENCAP'
    $pull = Invoke-NativeCapture -FilePath $adb -Arguments @('-s', $serial, 'pull', $remoteScreenshot, $screenshotPath)
    Assert-NativeSuccess -Result $pull -FailureClass 'FNX_PLAY_SCREENSHOT_02_PULL'
    [void](Invoke-NativeCapture -FilePath $adb -Arguments @('-s', $serial, 'shell', 'rm', '-f', $remoteScreenshot))

    if (-not (Test-Path -LiteralPath $screenshotPath -PathType Leaf)) {
        throw 'FNX_PLAY_SCREENSHOT_02_LOCAL_PNG_MISSING'
    }
    $png = Get-PngDimensions -Path $screenshotPath
    if ($png.Width -ne $TargetWidth -or $png.Height -ne $TargetHeight) {
        throw ('FNX_PLAY_SCREENSHOT_02_DIMENSION_MISMATCH_' + $png.Width + 'x' + $png.Height)
    }
    $screenshotBytes = (Get-Item -LiteralPath $screenshotPath).Length
    if ($screenshotBytes -lt 60000) {
        throw ('FNX_PLAY_SCREENSHOT_02_SUSPICIOUSLY_SMALL_' + $screenshotBytes)
    }
    $screenshotSha = (Get-FileHash -LiteralPath $screenshotPath -Algorithm SHA256).Hash.ToLowerInvariant()

    Write-Output 'SCREENSHOT_02_PHASE=RESTORE_PRODUCTION_APK'
    Install-Apk -Adb $adb -Serial $serial -Apk $productionApk -FailureClass 'FNX_PLAY_SCREENSHOT_02_PRODUCTION_RESTORE_FAILED'
    $restored = Assert-CanonicalReleaseInstalled -Adb $adb -Serial $serial -PackageId $PackageId -FailurePrefix 'FNX_PLAY_SCREENSHOT_02_PRODUCTION_RESTORE'
    ...
    $receipt = [ordered]@{
        schema_version = 1
        kind = 'FITNEXUS_PLAY_STORE_SCREENSHOT_02_RECEIPT'
        ...
        next_gate = [string]$Contract.next_gate
    }
    ...
    if ($null -ne $adb -and -not [string]::IsNullOrWhiteSpace([string]$serial)) {
        ...
        if ($captureInstalled -and -not $productionRestored -and -not [string]::IsNullOrWhiteSpace([string]$productionApk) -and (Test-Path -LiteralPath $productionApk -PathType Leaf)) {
            try {
                Write-Output 'SCREENSHOT_02_PHASE=FAILSAFE_RESTORE_PRODUCTION_APK'
                Install-Apk -Adb $adb -Serial $serial -Apk $productionApk -FailureClass 'FNX_PLAY_SCREENSHOT_02_FAILSAFE_RESTORE_INSTALL_FAILED'
                $failsafeRestored = Assert-CanonicalReleaseInstalled -Adb $adb -Serial $serial -PackageId $PackageId -FailurePrefix 'FNX_PLAY_SCREENSHOT_02_FAILSAFE_RESTORE'
...
if ($null -ne $failure) {
    Write-Output 'FITNEXUS_PLAY_STORE_SCREENSHOT_02_CAPTURE=FAIL'
...
Write-Output 'FITNEXUS_PLAY_STORE_SCREENSHOT_02_CAPTURE=PASS'
