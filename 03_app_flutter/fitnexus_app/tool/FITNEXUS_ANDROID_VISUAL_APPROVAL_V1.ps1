...
function Get-FreeEmulatorPort([string]$Adb) {
    ...
    try {
        ...
        # ADB isolation remains authoritative if listener inventory is unavailable.
    }

    for ($port = 5580; $port -le 5680; $port += 2) {
        if (-not $used.Contains($port) -and -not $used.Contains($port + 1)) {
            return $port
        }
    }
...
function Wait-NewEmulatorRegistered {
    param(
        ...
        [Parameter(Mandatory = $true)][System.Diagnostics.Process]$EmulatorProcess,
        [int]$TimeoutSeconds = 180
    )
    $deadline = [DateTime]::UtcNow.AddSeconds($TimeoutSeconds)
    while ([DateTime]::UtcNow -lt $deadline) {
        if ($EmulatorProcess.HasExited) {
            Fail ('NEW_EMULATOR_PROCESS_EXITED_' + $EmulatorProcess.ExitCode)
        }
        foreach ($device in @(Get-AdbDeviceSnapshot -Adb $Adb)) {
            if ($device.Serial -eq $Serial) {
                return
            }
        }
...
function Wait-NewEmulatorBooted {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        if ($EmulatorProcess.HasExited) {
            Fail ('NEW_EMULATOR_PROCESS_EXITED_DURING_BOOT_' + $EmulatorProcess.ExitCode)
        }
        ...
        if ($state.ExitCode -eq 0 -and $state.StdOut.Trim() -eq 'device') {
            ...
            if ($boot.ExitCode -eq 0 -and $boot.StdOut.Trim() -eq '1') {
                return
            }
        }
        Start-Sleep -Seconds 2
...
try {
    ...
    if ($install.ExitCode -ne 0 -or $install.StdOut -notmatch 'Success') {
        Fail ('ADB_INSTALL_FAILED_' + $install.ExitCode)
    }
    ...
    if ($focused.ExitCode -ne 0 -or $focused.StdOut -notmatch [regex]::Escape($PackageId)) {
        Fail 'FITNEXUS_NOT_FOREGROUND_ON_NEW_EMULATOR'
    }
